...
/*   Updated: 2021/02/06 22:28:37 by mchun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...

{
	...
	padd_len = -1;
	if (i->width > ft_digitlen_base(num, 16))
		padd_len = (i->width - ft_digitlen_base(num, 16));
	while (padd_len-- > 0)
		ft_putchar_fd('0', 1);
	if (num == LLONG_MIN)
	...
	return ((i->width > ft_digitlen_base(num, 16)) ? \
				i->width : ft_digitlen_base(num, 16));
}
...
static int		hex_normal(unsigned long long num, t_info *i)
{
	...
	digit_len = ft_digitlen_base(num, 16);
