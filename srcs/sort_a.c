...
/*   Updated: 2021/05/30 14:19:53 by mchun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/push_swap.h"

void	sort_a_less_n_equal_3(t_ll **ab_array, int sort_num)
{
	int		comp[3];
	...
	{
		if (comp[1] > comp[2] && comp[2] > comp[1])
			sort_a_132(ab_array);
...
int		**sort_a(t_ll **ab_array, int **piv_arr, int sort_num)
{
	...
	init_freq(&freq);
	node = ab_array[STACK_A]->head;							//partition_A 함수로 나눌 수 있을 듯..?
	while (sort_num-- > 0)
	{
