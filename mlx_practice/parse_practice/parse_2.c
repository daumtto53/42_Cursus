...
int		identifier_fc(t_conf *conf, int fd, char **splitstr, char *id)
{
	...
	if (ft_strncmp(id, "F", 1) == 0)
	{
		...
		if (!colorstr)
			return (-1);
		conf->color_floor = conf->color_floor | (ft_atoi(colorstr[0]) << RGB_R_OP);
		conf->color_floor = conf->color_floor | (ft_atoi(colorstr[1]) << RGB_G_OP);
		conf->color_floor = conf->color_floor | (ft_atoi(colorstr[2]) << RGB_B_OP);
	...
	if (ft_strncmp(id, "C", 1) == 0)
	{
		...
		if (!colorstr)
			return (-1);
		conf->color_ceil = conf->color_ceil | (ft_atoi(colorstr[0]) << RGB_R_OP);
		conf->color_ceil = conf->color_ceil | (ft_atoi(colorstr[1]) << RGB_G_OP);
		conf->color_ceil = conf->color_ceil | (ft_atoi(colorstr[2]) << RGB_B_OP);
		conf->complete_input |= (COLOR_CEIL);
		ft_split_free(colorstr);
	}
	return (1);
}
